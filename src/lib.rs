    ...
    pub fn recalculate_weights(
        ...
        for (neuron_num, neuron) in self.layers[0].iter_mut().enumerate() {
            for (weight_num, weight) in neuron.weights.iter_mut().enumerate() {
                let mut new_weight_change = self.alpha * errors[0][neuron_num] * input[weight_num];
                new_weight_change += MOMENTUM * neuron.last_weights_change[weight_num];
                *weight += new_weight_change;
                neuron.last_weights_change[weight_num] = new_weight_change;
            }
            let mut new_bias_change = self.alpha * errors[0][neuron_num];
            new_bias_change += MOMENTUM * neuron.last_bias_change;
            neuron.bias += new_bias_change;
            neuron.last_bias_change = new_bias_change;
        }
        ...
        for layer_num in 1..self.layers.len() {
            for (neuron_num, neuron) in self.layers[layer_num].iter_mut().enumerate() {
                for (weight_num, weight) in neuron.weights.iter_mut().enumerate() {
                    let mut new_weight_change = self.alpha * errors[layer_num][neuron_num] * all_neuron_outs[layer_num-1][weight_num];
                    new_weight_change += MOMENTUM * neuron.last_weights_change[weight_num];
                    *weight += new_weight_change;
                    neuron.last_weights_change[weight_num] = new_weight_change;
                }
                let mut new_bias_change = self.alpha * errors[layer_num][neuron_num];
                new_bias_change += MOMENTUM * neuron.last_bias_change;
                neuron.bias += new_bias_change;
                neuron.last_bias_change = new_bias_change;
            }
...
pub struct Neuron {
    pub weights: Vec<f64>,
    pub bias: f64,
    pub last_weights_change: Vec<f64>,
    pub last_bias_change: f64,
}
...
impl Neuron {
    pub fn new(inputs: usize) -> Neuron {
        ...
        let mut neuron = Neuron {
            ...
            last_weights_change: Vec::with_capacity(inputs),
            last_bias_change: 0.0
        };

        for _ in 0..inputs {
            neuron.weights.push(rng.gen_range(0.0, 1.0));
            neuron.last_weights_change.push(0.0);
        }
...
pub struct RsTrainingDataVec {
    training_data : Vec<TrainingData>,
}

impl RsTrainingDataVec {
    fn new() -> RsTrainingDataVec {
        RsTrainingDataVec { training_data: Vec::new()}
    }

    fn add(&mut self, inputs : Vec<f64>, expected : Vec<f64>) {
        self.training_data.push(TrainingData { inputs: inputs, expected: expected});
    }
}

#[no_mangle]
pub extern fn rs_training_data_vec_new() -> *mut RsTrainingDataVec {
    Box::into_raw(Box::new(RsTrainingDataVec::new()))
}

#[no_mangle]
pub unsafe extern fn rs_training_data_vec_add(p: *mut RsTrainingDataVec, inputs: *const c_double, inputs_len: i64, expected: *const c_double, expected_len: i64) {
    let ptr = {
        assert!(!p.is_null());
        &mut *p
    };
    let inputs_parsed : &[c_double] = slice::from_raw_parts(inputs, inputs_len as usize);
    let expected_parsed : &[c_double] = slice::from_raw_parts(expected, expected_len as usize);

    let mut inputs_vec: Vec<f64> = vec![];
    for i in inputs_parsed {
        inputs_vec.push(*i as f64);
    }
    let mut expected_vec: Vec<f64> = vec![];
    for i in expected_parsed {
        expected_vec.push(*i as f64);
    }

    ptr.training_data.push(TrainingData{inputs: inputs_vec, expected: expected_vec});
    println!("{:?}", ptr.training_data);
}

#[no_mangle]
pub extern fn rs_training_data_vec_free(p: *mut RsTrainingDataVec) {
    if p.is_null() {return}
    unsafe {Box::from_raw(p);}
}

#[no_mangle]
pub unsafe extern fn rs_run(p: *mut RsTrainingDataVec, alpha: c_double, input : i64, hidden: i64, output: i64, finish: c_double) {
    let ptr = {
        assert!(!p.is_null());
        &mut *p
    };
    let mut network = Network::new(alpha as f64, vec![hidden as usize, output as usize], input as usize);
    network.start_learning(ptr.training_data.clone(), finish as f64);
}
